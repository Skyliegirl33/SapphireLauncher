...
namespace SapphireBootWPF.Dalamud
{
    static class DalamudUpdater
    {
        ...
        public static void SetOverlayProgress()
        {
        ...
        public static void Run(DirectoryInfo gamePath)
        {
            Serilog.Log.Information("[DUPDATE] Starting...");

            Task.Run(() =>
            {
                try
                {
                    UpdateDalamud(gamePath);
                }
                catch (Exception ex)
                {
                    Serilog.Log.Information(ex, "[DUPDATE] Update failed...");
        ...
        private static void UpdateDalamud(DirectoryInfo gamePath)
        {
            ...
            Serilog.Log.Information("[DUPDATE] Now starting for Dalamud {0}", remoteVersionInfo.AssemblyVersion);

            if (!addonPath.Exists || !IsIntegrity(addonPath))
            {
                Serilog.Log.Information("[DUPDATE] Not found, redownloading");
                ...
                {
                    Serilog.Log.Information(ex, "[DUPDATE] Could not download update package.");

                    State = DownloadState.NoIntegrity;
                    return;
                }

                Serilog.Log.Information("[DUPDATE] Download OK!");
            }

            if (remoteVersionInfo.RuntimeRequired || settings.DoDalamudRuntime)
            {
                Serilog.Log.Information("[DUPDATE] Now starting for .NET Runtime {0}", remoteVersionInfo.RuntimeVersion);
            ...
            try
            {
                ...
                {
                    Serilog.Log.Information("[DUPDATE] Assets not ensured, bailing out...");
                    State = DownloadState.Failed;
                    return;
                }
            }
            ...
            {
                Serilog.Log.Information(ex, "[DUPDATE] Asset ensurement error, bailing out...");
                State = DownloadState.Failed;
                return;
            }
            ...
            {
                Serilog.Log.Information("[DUPDATE] Integrity check failed.");
            ...
            Serilog.Log.Information("[DUPDATE] All set for " + remoteVersionInfo.SupportedGameVer);
        ...
        public static bool IsIntegrity(DirectoryInfo addonPath)
        {
            ...
            {
                Serilog.Log.Information(ex, "[DUPDATE] No dalamud integrity.");
                return false;
            }
        ...
        private static void Download(DirectoryInfo addonPath, DalamudSettings settings)
        {
            ...
            {
                Serilog.Log.Information(ex, "[DUPDATE] Could not copy to dev folder.");
            }
        }
